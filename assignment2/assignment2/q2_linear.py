...
class Linear(DQN):
    ...
    def add_loss_op(self, q, target_q):
        ...
        q_sample = self.r + not_done * config.gamma * tf.reduce_max(target_q, axis=1)
        indices = tf.one_hot(self.a, num_actions)
        q = tf.reduce_sum(indices * q, axis=1)
    ...
    def add_optimizer_op(self, scope):
        ...
        col = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope)
        optimizer = tf.train.AdamOptimizer(learning_rate=self.lr)
        grads = optimizer.compute_gradients(self.loss, col)
        if config.grad_clip:
            for i, (g, v) in enumerate(grads):
                if g is not None:
                    grads[i] = (tf.clip_by_norm(g, config.clip_val), v)
        self.train_op = optimizer.apply_gradients(grads)
        grad_list = [g for (g, v) in grads if g is not None]
